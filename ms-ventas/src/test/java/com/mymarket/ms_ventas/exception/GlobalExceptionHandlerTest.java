package com.mymarket.ms_ventas.exception;
...
class GlobalExceptionHandlerTest {
    ...
    void handleNotFound_debeRetornar404() {
        when(request.getRequestURI()).thenReturn("/api/ventas/99");
        EntityNotFoundException ex = new EntityNotFoundException("Venta no encontrada");
        ...
        assertEquals("Venta no encontrada", response.getBody().getMessage());
        assertEquals("/api/ventas/99", response.getBody().getPath());
    }
    ...
    void handleValidation_debeRetornar400ConErroresDeCampo() {
        ...
        FieldError fieldError = new FieldError("venta", "productoId", "no puede estar vacio");
