package com.mymarket.ms_reportes.exception;
...
class GlobalExceptionHandlerTest {
    ...
    void handleNotFound_debeRetornar404() {
        when(request.getRequestURI()).thenReturn("/api/reportes/99");
        EntityNotFoundException ex = new EntityNotFoundException("Reporte no encontrado");
        ...
        assertEquals("/api/reportes/99", response.getBody().getPath());
    }

    @Test
    void handleBadRequest_debeRetornar400() {
        when(request.getRequestURI()).thenReturn("/api/reportes");
    ...
    void handleValidation_debeRetornar400ConErroresDeCampo() {
        ...
        FieldError fieldError = new FieldError("reporte", "tipo", "no puede estar vacio");
