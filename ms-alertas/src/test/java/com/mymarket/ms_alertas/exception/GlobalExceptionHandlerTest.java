package com.mymarket.ms_alertas.exception;
...
class GlobalExceptionHandlerTest {
    ...
    void handleNotFound_debeRetornar404() {
        when(request.getRequestURI()).thenReturn("/api/alertas/99");
        EntityNotFoundException ex = new EntityNotFoundException("Alerta no encontrada");
        ...
        assertEquals("Alerta no encontrada", response.getBody().getMessage());
        assertEquals("/api/alertas/99", response.getBody().getPath());
    }

    @Test
    void handleBadRequest_debeRetornar400() {
        when(request.getRequestURI()).thenReturn("/api/alertas");
    ...
    void handleValidation_debeRetornar400ConErroresDeCampo() {
        ...
        FieldError fieldError = new FieldError("alerta", "tipo", "no puede estar vacio");
