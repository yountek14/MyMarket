package com.mymarket.ms_proveedores.exception;
...
class GlobalExceptionHandlerTest {
    ...
    void handleNotFound_debeRetornar404() {
        when(request.getRequestURI()).thenReturn("/api/proveedores/99");
        EntityNotFoundException ex = new EntityNotFoundException("Proveedor no encontrado");
        ...
        assertEquals("Proveedor no encontrado", response.getBody().getMessage());
        assertEquals("/api/proveedores/99", response.getBody().getPath());
    }

    @Test
    void handleBadRequest_debeRetornar400() {
        when(request.getRequestURI()).thenReturn("/api/proveedores");
    ...
    void handleValidation_debeRetornar400ConErroresDeCampo() {
        ...
        FieldError fieldError = new FieldError("proveedor", "nombre", "no puede estar vacio");
