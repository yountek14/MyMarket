package com.mymarket.ms_pedidos.exception;
...
class GlobalExceptionHandlerTest {
    ...
    void handleNotFound_debeRetornar404() {
        when(request.getRequestURI()).thenReturn("/api/pedidos/99");
        EntityNotFoundException ex = new EntityNotFoundException("Pedido no encontrado");
        ...
        assertEquals("/api/pedidos/99", response.getBody().getPath());
    }
    ...
    void handleValidation_debeRetornar400ConErroresDeCampo() {
        ...
        FieldError fieldError = new FieldError("pedido", "proveedorId", "no puede estar vacio");
