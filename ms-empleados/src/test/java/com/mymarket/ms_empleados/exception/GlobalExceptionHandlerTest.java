package com.mymarket.ms_empleados.exception;
...
class GlobalExceptionHandlerTest {
    ...
    void handleNotFound_debeRetornar404() {
        when(request.getRequestURI()).thenReturn("/api/empleados/99");
        EntityNotFoundException ex = new EntityNotFoundException("Empleado no encontrado");
        ...
        assertEquals("/api/empleados/99", response.getBody().getPath());
    }
    ...
    void handleValidation_debeRetornar400ConErroresDeCampo() {
        ...
        FieldError fieldError = new FieldError("empleado", "nombre", "no puede estar vacio");
