...
class GlobalExceptionHandlerTest {
    ...
    void handleNotFound_debeRetornar404() {
        when(request.getRequestURI()).thenReturn("/api/inventario/99");
        EntityNotFoundException ex = new EntityNotFoundException("Inventario no encontrado");
        ...
        assertEquals("Inventario no encontrado", response.getBody().getMessage());
        assertEquals("/api/inventario/99", response.getBody().getPath());
    }

    @Test
    void handleBadRequest_debeRetornar400() {
        when(request.getRequestURI()).thenReturn("/api/inventario");
    ...
    void handleValidation_debeRetornar400ConErroresDeCampo() {
        ...
        FieldError fieldError = new FieldError("inventario", "stockActual", "no puede estar vacio");
